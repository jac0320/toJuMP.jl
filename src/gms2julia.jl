function gms2julia(gmsName::AbstractString, juliaName::AbstractString, mode::AbstractString="raw"; kwargs...)
...
function read_equation(file::IOStream, gms::oneProblem, lInit::AbstractString; kwargs...)
    ...
    if eName in gms.rows   # TODO: This extra assertion can be elimated
        ...
        while true # Concatenate the parsed elements
            ...
            if ismatch(r"=\w=", sl[i]) && isempty(sense)
                sense = split(sl[i], "=")[2]
                gms.rowsSense[eName] = sense    # Storing raw sense character
                i = i + 1
            elseif ismatch(r"=\w=", sl[i]) && !isempty(sense)
            ...
            elseif !ismatch(r"=\w=", sl[i]) && isempty(sense)
                ...
                # A week assertion :: eqach equation (stripped) ends with rhs
                @assert i == length(sl)
